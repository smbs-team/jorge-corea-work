...
namespace D2SSyncHelpers.Services
{
    ...
    public class DynamicsSecurityBase
    {
        ...
        /// <param name="addSyncRequest">Add sync request to header.</param>
        /// <returns>Response String.</returns>
        protected async Task<string> GetContent(Uri myUri, bool addSyncRequest = false)
        {
            Console.WriteLine("Getting content from Uri: " + myUri.ToString());
            var client = new HttpClient();
            this.SetupHeaders(client, addSyncRequest);
            ...
            throw new Exception("Failed. " + content);
        }

        /// <summary>
        /// Post content to API.
        /// </summary>
        /// <param name="myUri">Uri to post to.</param>
        /// <param name="jSONPayload">Payload to save.</param>
        /// <returns>A <see cref="Task"/> representing the result of the asynchronous operation.</returns>
        protected async Task PostContent(Uri myUri, string jSONPayload)
        {
            Console.WriteLine("Posting content to Uri: " + myUri.ToString());
            var httpContent = new StringContent(jSONPayload, Encoding.UTF8, "application/json");
            using var client = new HttpClient();
            this.SetupHeaders(client, false);

            var response = await client.PatchAsync(myUri, httpContent);
            var content = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception("Failed. " + content);
        ...
        /// <param name="addSyncRequest">Add sync request to header.</param>
        protected virtual void SetupHeaders(HttpClient httpClient, bool addSyncRequest = false)
        {
            ...
            if (addSyncRequest)
            {
                httpClient.DefaultRequestHeaders.Add("Prefer", "odata.track-changes");
            }
            else
            {
