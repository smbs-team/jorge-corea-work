        ...
        public void ComparePK(string sqlConnectionString, EntityModel entity, long uploadTicket, IConnector connector)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(sqlConnectionString))
                {
                    ...
                    policy.Execute(() =>
                    {
                        ...
                        using (SqlCommand command = new SqlCommand(query, con))
                        {
                            ...
                            while (reader.Read())
                            {
                                pairs.Add(string.Empty + Guid.Parse(reader["guid_mb"].ToString()), string.Empty + reader[this.pkName].ToString());
                            }
                        }
                    });
                    this.countOfRetries = 1;
                ...
                foreach (Dictionary<string, object> keyValues in tempList)
                {
                    foreach (var item in keyValues)
                    {
                        if (item.Key.ToLower().Equals(this.pkName.ToLower()) && !pairs.ContainsValue(item.Value.ToString().ToLower()))
                        {
