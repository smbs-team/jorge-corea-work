...
namespace D2SSyncHelpers.Services
{
    ...
    public class DBTablesService : IDBTablesService
    {
        ...
        private const string GetSchema = @"
            SELECT        schema_name(schema_id) AS schema_name, name AS table_name, create_date, modify_date
            FROM            sys.tables AS t
            WHERE        (schema_name(schema_id) = 'dynamics') AND name NOT LIKE '%_snapshot%'
            ORDER BY table_name
            ";
...
";
        ...
        if (snapEntities.Contains(table.Name.ToLower()))
        {
           await this.dataAccess.SaveData<object>(table.CreateScript(true), null);
        }

        await this.dataAccess.SaveData<object>(table.CreateScript(false), null);
        }
        ...
        public async Task<TableChange> GetLastSavedInfo(string entityName)
        {
            ...
            lastSavedInfo.TableId = entityName;
            lastSavedInfo.LastDateProcessed = lastDate;
            lastSavedInfo.LastGuidProcessed = lastGuid;
        ...
        /// <returns>Task.</returns>
        public Task<SaveError> BulkInsert(SqlConnection connection, JArray actualValues, DBTable destTable)
        {
            ...
            catch (JsonException ex)
            {
                ...
                foreach (JObject item in actualValues)
                {
                    ...
                    for (int i = 0; i < destTable.Fields.Count(); i++)
                    {
                        if (destTable.Fields.AsList()[i].DataType.Equals("int") && list[i].val != null)
                        {
                            row[list[i].field] = int.Parse(list[i].val);
                        }
                        else if (destTable.Fields.AsList()[i].DataType.Equals("bigint") && list[i].val != null)
                        {
                            row[list[i].field] = long.Parse(list[i].val);
                        }
                        else if (destTable.Fields.AsList()[i].DataType.Equals("bit") && list[i].val != null)
                        {
                            if (int.Parse(list[i].val) == 0)
                            {
                                row[list[i].field] = false;
                            }
                            else
                            {
                                row[list[i].field] = true;
                            }
                        }
                        else
                        {
                            row[list[i].field] = list[i].val;
                        }
                    }
            ...
            foreach (string columnName in this.DistinctColumnsOrigin(dataTable, destTable))
            {
                dataTable.Columns.Remove(columnName);
            }

            foreach (string columnName in this.DistinctColumnsDestiny(dataTable, destTable))
            {
                ...
                {
                    dataTable.Columns.Add(columnName, typeof(string)); // Ver como mandarle el tipo bien desde el principio
                }
            }
            ...
            using (SqlBulkCopy bulkInsert = new SqlBulkCopy(connection))
            {
                ...
                try
                {
                    ...
                    bulkInsert.WriteToServer(dtCloned);
                    connection.Close();
                }
                catch (Exception ex)
                {
                    ...
                    connection.Close();
                    return Task.FromResult(new SaveError
                    {
                        HadError = true,
                        ErrorMessage = ex.Message + " " + ex.InnerException?.Message,
                        ErrorType = ex.GetType().ToString(),
            ...
            return Task.FromResult(new SaveError { HadError = false });
        }
        ...
        private void ChangeDataColumns(DataTable dtCloned, DBTable destTable)
        {
            for (int i = 0; i < dtCloned.Columns.Count; i++)
            {
                if (destTable.Fields.Cast<DBField>().ToList()[i].DataType.Contains("char") ||
