namespace CustomSearchesServicesLibrary.CustomSearches.Services.CustomImporters
{
    ...
    /// <summary>
    /// Service that deletes the land model post processes.
    /// </summary>
    public class DeleteLandModelService : BaseService
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="DeleteLandModelService"/> class.
        /// </summary>
        /// <param name="serviceContext">The service context.</param>
        public DeleteLandModelService(IServiceContext serviceContext)
            : base(serviceContext)
        {
        }
        ...
        public async Task DeleteLandModelAsync(
            Guid datasetId,
            ...
            ILogger logger)
        {
            ...
            var datasetPostProcesses =
                await (from up in dbContext.UserProject
                       ...
                       where upd.OwnsDataset == true &&
                        ...
                        dpp.PostProcessRole != null &&
                        PhysicalInspectionProjectBusinessLogic.LandSchedulePostProcessRoleOrder.Contains(dpp.PostProcessRole.Trim().ToLower())
                       select dpp).
                    ToListAsync();

            InputValidationHelper.AssertNotEmpty(datasetPostProcesses, nameof(datasetPostProcesses), $"The dataset '{datasetId}' doesn't have land model post processes.");
            ...
            var primaryLastLandModelDatasetPostProcess =
                datasetPostProcesses.Where(dpp => (dpp.DatasetId == datasetId) &&
                (dpp.PostProcessRole?.Trim().ToLower() == PhysicalInspectionProjectBusinessLogic.LandSchedulePostProcessRoleOrder.Last().ToLower()))
                .FirstOrDefault();

            var primaryDataset = primaryLastLandModelDatasetPostProcess.Dataset;
            ...
            this.ServiceContext.AuthProvider.AuthorizeProjectOrFolderItemOperation<Folder>(primaryDataset.UserId, primaryDataset.ParentFolder, primaryDataset.IsLocked, userProject, "DeleteLandModel");
            ...
            await DatasetHelper.GetAlterDatasetLockAsyncV2(
                async (datasetState, datasetPostProcessState) =>
                {
                    ...
                    int landScheduleLength = PhysicalInspectionProjectBusinessLogic.LandSchedulePostProcessRoleOrder.Count;
                    for (int i = landScheduleLength - 1; i >= 0; i--)
                    {
                        string roleName = PhysicalInspectionProjectBusinessLogic.LandSchedulePostProcessRoleOrder[i];
