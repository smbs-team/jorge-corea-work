namespace CustomSearchesFunctions.CustomSearches.Functions.RScriptModel
{
    ...
    /// </summary>
    public class UploadRScriptFile
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UploadRScriptFile"/> class.
        /// </summary>
        ...
        public UploadRScriptFile(IFactory<CustomSearchesDbContext> dbContextFactory, ICloudStorageProvider cloudStorageProvider, IServiceContextFactory serviceContextFactory)
        {
        ...
        /// <param name="FileName" cref="string" in="header">The file name.</param>
        ...
        [FunctionName("UploadRScriptFile")]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "API/CustomSearches/UploadRScriptFile/{modelId}")] HttpRequest req, int modelId, ILogger log)
        {
            return await CustomSearchesFunctionsExceptionHandler.GlobalExceptionHandler(
                async () =>
                {
                    if (req.Headers.ContainsKey("FileName") == false)
                    {
                        throw new ArgumentNullException("FileName in header");
                    }
                    ...
                    string fileName = req.Headers["FileName"][0];

                    IServiceContext serviceContext = await this.serviceContextFactory.CreateFromHttpRequestAsync(req, log);
                    UploadRScriptFileService service = new UploadRScriptFileService(serviceContext);
                    using (CustomSearchesDbContext dbContext = this.dbContextFactory.Create())
                    {
                        CloudBlobContainer blobContainer = await this.cloudStorageProvider.GetCloudBlobContainer(UploadRScriptFile.RScriptBlobContainerName, serviceContext.AppCredential);
                        await service.UploadRScriptFileAsync(modelId, fileName, req.Body, dbContext, blobContainer);
