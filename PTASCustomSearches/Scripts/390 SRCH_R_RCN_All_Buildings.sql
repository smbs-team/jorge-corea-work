...
DROP TABLE IF EXISTS #PrevParcels;
...
--Hairo comment: insert a record for each building
INSERT #Parcels
 (
 PIN
,MapPin 
...
UPDATE #Parcels
   ...
   INNER JOIN dynamics.vw_ResBldg b 
      ON rs.ParcelId = b._ptas_parceldetailid_value 
	  AND rs.BldgGuid = b.ptas_buildingdetailid 
   LEFT JOIN [dynamics].[ptas_year] y1
      ON b._ptas_yearbuiltid_value = y1.ptas_yearid
   LEFT JOIN [dynamics].[ptas_year] y2
      ON b._ptas_yearrenovatedid_value = y2.ptas_yearid
...
INNER JOIN #Parcels rbd 
   ...
   AND bd.ptas_buildingdetailid = rbd.BldgGuid
...
SELECT bd._ptas_parceldetailid_value,bd.ptas_buildingdetailid
    ...
    INNER JOIN #Parcels rbd 
    ON bd._ptas_parceldetailid_value = rbd.ParcelId
    AND bd.ptas_buildingdetailid = rbd.ParcelId
    INNER JOIN [dynamics].[ptas_propertytype] dpt
    ON bd._ptas_propertytypeid_value = dpt.ptas_propertytypeid
    AND dpt.ptas_description = 'Residential'		
...
SELECT a._ptas_parceldetailid_value
 ...
 --SELECT * FROM #Accessory ORDER BY BldgGuid
...
--SELECT * FROM #BldgsForRealPropValEst
	...
	SET AllBldgRCN = ISNULL((SELECT SUM(sBldgRCN) FROM #BldgsForRealPropValEst e
	                  WHERE e.ParcelId = rbd.ParcelId AND e.BldgGuid =rbd.BldgGuid ),0)
...
--SELECT @CostBillYr , a.EffYr ,a.AccyType,@CostBillYr - 1 - a.EffYr FROM #Accessory a
...
BEGIN
	SELECT 
		   ...
		   ,p.TotalRCN
		   ,p.TotalRCNLD
		   ,MainTotalRCN =  CASE WHEN p.BldgNbr= 1 THEN (SELECT SUM(p1.TotalRCN) FROM #Parcels p1 WHERE P.ParcelId = p1.ParcelId )
							ELSE 0 
							END
		   ,MainTotalRCNLD = CASE WHEN p.BldgNbr= 1 THEN (SELECT SUM(p1.TotalRCNLD) FROM #Parcels p1 WHERE P.ParcelId = p1.ParcelId )
							ELSE 0 
							END 
	
	FROM #Parcels p
	ORDER BY p.Major,p.Minor,p.BldgNbr
END 	
