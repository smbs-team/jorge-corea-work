...
export class PortalContact {
  ...
  businessAccess: {
    id: string;
    name: string; //Business name (from personal property)
  }[];
  ...
  updateFields = (newData: Partial<PortalContact>): void => {
    ...
    Object.defineProperties(thisRef, propertiesUpdated);
  };
...
export interface PortalAddress {
  ...
  isSaved: boolean;
  title: string;
  line1: string; //Holds street name (ptas_streetname)
  line2: string;
  countryId: string;
  state: string;
  stateId: string;
