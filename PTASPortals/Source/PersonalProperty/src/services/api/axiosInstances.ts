...
export const axiosPerPropInstance = axios.create({
  timeout: 60 * 2 * 1000,
  baseURL: `${process.env.REACT_APP_PERSONAL_PROPERTY_API_URL}`,
  headers: {
    Authorization: `Bearer ${process.env.REACT_APP_MAGIC_TOKEN_TEMP}`,
  },
});
...
axiosPerPropInstance.interceptors.response.use((res) => res, exceptionError);
