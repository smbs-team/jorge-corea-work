 ...
 */
import { BasicAddressData } from 'models/addresses';
import {
  PortalAddress,
  PortalContact,
  PortalEmail,
...
import { StateEntity, CityEntity, ZipCodeEntity } from 'models/addresses';
...
class ContactApiService {
      ...
      requests: [
        {
          ...
          query: `$filter=ptas_email eq 'ringo@test.com'
          &$expand=ptas_portalcontact`,
        },
      ],
    };
  ...
  //TODO: delete. TEMP method for development
  getTestContactSec = (): Promise<
    ApiServiceResult<PortalContact | undefined>
  > => {
    return this.getPortalContactById('4c54a667-70fd-43cb-941b-1cdff1eed1ee');
  };
  ...
  /**
   * Gets portal contact
   * @param contactId - ID of portal contact
   */
  getPortalContactById = (
    contactId: string
  ): Promise<ApiServiceResult<PortalContact | undefined>> => {
    const data = {
      requests: [
        {
          ...
          query: `$filter=ptas_portalcontactid eq '${contactId}'`,
        },
        {
          entityName: 'ptas_portalemail',
          query: `$top=1&$filter=_ptas_portalcontact_value eq '${contactId}' and ptas_primaryemail eq true`,
        },
      ],
    };
    ...
    return handleReq(async () => {
      ...
      const res = (
        await axiosPerPropInstance.post<{
          items: {
            entityName: string;
            changes: PortalContactEntity | PortalEmailEntity;
          }[];
        }>(url, data)
      ).data;

      let contact: PortalContact | undefined;
      let email: PortalEmail | undefined;
      if (res.items.length) {
        res.items.forEach((item) => {
          if (item.entityName === 'ptas_portalcontact') {
            contact = new PortalContact(item.changes as PortalContactEntity);
          } else if (item.entityName === 'ptas_portalemail') {
            const emailEntity = item.changes as PortalEmailEntity;
            email = {
              id: emailEntity.ptas_portalemailid,
              isSaved: true,
              email: emailEntity.ptas_email,
              primaryEmail: emailEntity.ptas_primaryemail,
              portalContactId: emailEntity._ptas_portalcontact_value,
            };
          }
        });
      }
    ...
    return handleReq(async () => {
      ...
      result.items.forEach((item) => {
        if (item.entityName === 'ptas_stateorprovince') {
          ...
          addressData.stateId = (
            item.changes as StateEntity
          ).ptas_stateorprovinceid;
          ...
          addressData.zipCodeId = (
            item.changes as ZipCodeEntity
          ).ptas_zipcodeid;
        }
      });
  ...
  > => {
    const emailsReq = {
      entityName: 'ptas_portalemail',
      query: `$filter=_ptas_portalcontact_value eq '${portalContactId}'&$select=ptas_portalemailid`,
    };
    const addressesReq = {
      entityName: 'ptas_portaladdress',
      query: `$filter=_ptas_portalcontactid_value eq '${portalContactId}'&$select=ptas_portaladdressid`,
    };
    const phonesReq = {
      entityName: 'ptas_phonenumber',
      query: `$filter=_ptas_portalcontact_value eq '${portalContactId}'&$select=ptas_phonenumberid`,
    };
    ...
    const idsRes = await apiService.getGenericDynamicsAll<{
      ptas_portalemailid?: string;
      ptas_portaladdressid?: string;
      ptas_phonenumberid?: string;
    }>([emailsReq, addressesReq, phonesReq]);

    if (idsRes.data) {
      idsRes.data.forEach((el) => {
        if (el.ptas_portalemailid) {
          deleteReqs.push({
            entityName: 'ptas_portalemail',
            entityId: el.ptas_portalemailid,
          });
        } else if (el.ptas_portaladdressid) {
          deleteReqs.push({
            entityName: 'ptas_portaladdress',
            entityId: el.ptas_portaladdressid,
          });
        } else if (el.ptas_phonenumberid) {
          deleteReqs.push({
            entityName: 'ptas_phonenumber',
            entityId: el.ptas_phonenumberid,
          });
        }
      });
    }
    ...
    return handleReq(async () => {
      ...
      >({
        data: result.data.results?.length ? result.data.results : undefined,
      });
    });
  };
}
